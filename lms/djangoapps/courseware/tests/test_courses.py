...
class CourseInstantiationTests(ModuleStoreTestCase):
    ...
    def test_repeated_course_module_instantiation(self, loops, default_store, course_depth):
        ...
        with modulestore().default_store(default_store):
            ...
            chapter = ItemFactory(parent=self.course, category='chapter', graded=True)
        ...
        self.course = modulestore().get_course(self.course.id, depth=course_depth)

        for x in xrange(loops):
            field_data_cache = FieldDataCache.cache_for_descriptor_descendents(
                self.course.id, self.user, self.course, depth=course_depth
            )
            course_module = get_module_for_descriptor(self.user, fake_request, self.course, field_data_cache, self.course.id)
